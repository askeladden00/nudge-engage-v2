...
- **Success metric:** `project.md` still lists Success Metrics as undefined by the team. Before shipping even the smallest slice, we need agreement on what number moves (and by how much) to call it working. **Update:** real week-5 experiment data now exists (`data/metric-findings.md`) — day-30 retention lift vs. control (+14pts, 22%→36%) is a measurable candidate, though not yet statistically significant at the current sample size. A properly powered follow-up needs 1,158 users/arm (95% confidence, 80% power, 5pt MDE — see `data/experiment-design.md`). **Second update, from `data/metric-diagnosis.md`:** the specific metric proposed elsewhere ("% of users who take an action within 24hrs") may be the wrong one to optimize — users who acted on the nudge retained *worse* (31.0%) than users who opened but didn't act (42.1%). Unconfirmed and possibly a selection effect, not yet causal, but worth resolving before this metric gets locked in.
